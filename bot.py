...
async def on_message(message):
    ...
    if message.content.startswith('!givepoint'):
        ...
        with open('strikes.ini','w') as configfile:
            strikes_config.write(configfile)
        await client.send_message(message.channel, 'Point given to ' + strike_user.mention + '. He now has ' + str(strikes) + ' points.')
    if message.content.startswith('!removepoint'):
        ...
        with open('strikes.ini','w') as configfile:
            strikes_config.write(configfile)
        await client.send_message(message.channel, 'Point removed from ' + r_strike_user.mention + '. He now has '  + str(r_strikes) + ' points.')
